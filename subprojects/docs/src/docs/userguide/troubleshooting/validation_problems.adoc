...
This error indicates that a file collection is used as an input of a task, but that resolution failed.
In general, this shouldn't happen.
However, when this happens it means that Gradle cannot properly determine the task dependencies.

As a workaround, you can declare an explicit task dependency instead using `Task#dependsOn`.
